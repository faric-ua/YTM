...
## Generated historical matrix

The live repository-wide coverage inventory is:
...
The matrix reports current-standard file coverage for every version folder.
Historical missing artifacts are labeled `RETRO GAP`; active-release final-only
artifacts are labeled `PENDING`.

This makes gaps visible without fabricating old evidence.
