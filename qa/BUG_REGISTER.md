...
| BUG-033 | FIX IMPLEMENTED — WAVE 3 PHONE RETEST NEEDED | P2 | DataActivity modal windows used the shared UiChrome renderer but not a shared lifecycle owner, so ordinary Data confirmations disappeared on rotation while History modals survived. Wave 3 introduces RestorableModalController and routes all 11 Data modal states through one semantic save/restore contract. | Wave 2 broader UI smoke finding → Wave 3 static/full preflight |
