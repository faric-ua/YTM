...
| BUG-034 | FIX IMPLEMENTED — WAVE 3 R1 PHONE RETEST NEEDED | P2 | Post-operation Data result modal `History відновлено` disappeared on rotation. R1 hardens RestorableModalController so paused/state-saved/configuration teardown cannot clear durable semantic modal state; normal resumed user dismiss still clears it. | Wave 3 final-QA repro / screenshot evidence → Wave 3 R1 static/full preflight |
