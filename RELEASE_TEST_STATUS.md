...
| v1.4.40 | **PARTIALLY PHONE-TESTED — RENDERING PASS; ROTATION SCROLL FIX NEEDS RETEST** | Real-phone QA passed About entry, full release-history rendering/scrolling, readable markdown cleanup, and top-bar navigation. The oldest visible release is correctly v1.4.6 because that is the oldest `##` section in root `CHANGELOG.md`. User accepted distinct Back behavior: top-bar arrow steps History → About → Service, while system Back may exit Service directly to app Home. Rotation kept the History page but reset the list to v1.4.40; code now preserves/restores changelog scroll state and requires a signed-APK phone retest. |