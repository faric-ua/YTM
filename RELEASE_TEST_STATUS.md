...
| v1.4.40 | **PARTIALLY PHONE-TESTED — ABOUT ENTRY PASS; HISTORY PAGE QA CONTINUES** | Real-phone combined QA: `Меню → Сервіс → Про YTM Importer` exposes the expected release-history entry while Quick Start and Privacy remain present. In-app release history starts at v1.4.6 because v1.4.6 is currently the oldest `##` release section in root `CHANGELOG.md`; this matches the source-of-truth. Full history-page rendering, Back stack and rotation checks are still pending. |