...
| v1.4.41 | **PARTIALLY PHONE-TESTED — BROAD REAL-PHONE COVERAGE, NOT EXHAUSTIVE** | Signed v1.4.41 is installed on the real phone. Portrait account dialog PASS: readable profile copy, one-line `Змінити` on the left and `Закрити` on the right. Existing-playlist destination list also renders and uses direct row tap as the selection action by design; no footer buttons are required on that single-select screen. The project has broad accumulated real-phone coverage across many workflows, but v1.4.41 is **not** claimed as an exhaustive full-app regression; BUG-004 real-401, BUG-010 quota-preserving Restore and UX-017 fallback-title are still pending. UX-018 has representative account + destructive-confirmation phone PASS, but not exhaustive modal coverage. |