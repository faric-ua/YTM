...
class YouTubeApi {
    ...
    fun search(accessToken: String, track: Track): List<SearchCandidate> {
        ...
        val url = "https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=5&q=$q"
        ...
        for (i in 0 until items.length()) {
            ...
            val score = MatchScorer.score(track.originalArtist, track.originalTitle, title, channel)
            result += SearchCandidate(videoId, title, channel, score)
        }
    ...
    private fun request(method: String, url: String, accessToken: String, body: String? = null): ApiResponse {
    ...
    private fun requireSuccess(response: ApiResponse, action: String) {
        ...
        throw IllegalStateException("$action: HTTP ${response.code}${if (message.isNotBlank()) " — $message" else ""}")
    }
