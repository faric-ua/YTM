...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        listView.setOnItemClickListener { _, _, position, _ -> showTrackDialog(visibleTracks[position]) }
    ...
    private fun loadFile(uri: Uri) {
        ...
        runCatching { PlaylistParser.parse(fileName, text) }
            .onSuccess {
                ...
                updateSummary()
                status("Файл імпортовано. Тепер підключіть Google і натисніть «Знайти».")
            }
        ...
        Identity.getAuthorizationClient(this)
            ...
            .addOnSuccessListener { result ->
                if (result.hasResolution()) {
                    ...
                    val token = result.accessToken
                    if (token.isNullOrBlank()) toast("Google не повернув access token")
                    else {
                        accessToken = token
            ...
            .addOnFailureListener { e -> toast("Авторизація Google: ${e.message}") }
    }
        ...
        authorize {
            ...
            executor.execute {
                for ((index, track) in p.tracks.withIndex()) {
                    ...
                    refreshRow(index)
                    try {
                        val candidates = api.search(token, track)
                        ...
                        if (best == null) {
                            ...
                            track.status = when {
                                ...
                                best.score >= 0.48 -> TrackStatus.REVIEW
                        ...
                        if (e.message.orEmpty().contains("quota", ignoreCase = true)) {
                            runOnUiThread { toast("Закінчилась денна квота пошуку YouTube API") }
                            break
                        }
                    }
                    runOnUiThread {
                        progress.progress = index + 1
                        status("Пошук ${index + 1}/${p.tracks.size}…")
                        adapter.notifyDataSetChanged()
                        updateSummary()
                    }
                }
                runOnUiThread {
                    ...
                    status("Пошук завершено. Жовті треки краще перевірити натисканням.")
                    updateSummary()
                }
            }
        }
    ...
    private fun createPlaylist() {
        val p = playlist ?: return toast("Спочатку виберіть файл")
        val selected = p.tracks.filter { !it.selectedVideoId.isNullOrBlank() && it.status != TrackStatus.SKIPPED }
        ...
        if (questionable > 0) {
            AlertDialog.Builder(this)
                ...
                .setMessage("Можна створити плейлист зараз, але краще переглянути жовті позиції. Продовжити?")
                .setNegativeButton("Перевірю") { _, _ -> }
                .setPositiveButton("Продовжити") { _, _ -> actuallyCreatePlaylist(p, selected) }
                .show()
        } else actuallyCreatePlaylist(p, selected)
    }

    private fun actuallyCreatePlaylist(p: ImportedPlaylist, selected: List<Track>) {
        authorize {
            val token = accessToken ?: return@authorize
            progress.visibility = View.VISIBLE
            ...
            executor.execute {
                try {
                    ...
                    runOnUiThread { progress.progress = 1 }
                    ...
                    runOnUiThread {
                        ...
                        AlertDialog.Builder(this)
                            ...
                            .setMessage("Додано ${selected.count { it.status == TrackStatus.ADDED }} треків. Відкрити в YouTube Music?")
                            .setNegativeButton("Пізніше", null)
                            .setPositiveButton("Відкрити") { _, _ -> openInYtm() }
                            .show()
                    }
                } catch (e: Exception) {
                    runOnUiThread {
    ...
    private fun showTrackDialog(track: Track) {
        ...
        candidates.forEach { c ->
            labels += "${(c.score * 100).roundToInt()}%  ${c.title}\n${c.channelTitle}"
        }
        ...
        AlertDialog.Builder(this)
            ...
            .setItems(labels.toTypedArray()) { _, which ->
                when {
                    ...
                    which == candidates.size -> showPasteUrlDialog(track)
                    else -> {
    ...
    private fun showPasteUrlDialog(track: Track) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Використати") { _, _ ->
                ...
                if (videoId == null) toast("Не бачу YouTube video ID у посиланні")
                else {
    ...
    private fun applyCandidate(track: Track, candidate: SearchCandidate, manual: Boolean) {
    ...
    private fun openInYtm() {
        ...
        runCatching { startActivity(intent) }.onFailure { toast("Не вдалося відкрити ютм") }
    }

    private fun copyReplacements() {
        val p = playlist ?: return toast("Немає імпортованого плейлиста")
        val replacements = p.tracks.filter { it.manuallySelected }.joinToString("\n") { t ->
            val replacement = when {
                t.status == TrackStatus.SKIPPED -> "[пропущено]"
                t.selectedTitle == "Ручне посилання" -> "[ручне YouTube-посилання]"
                !t.selectedTitle.isNullOrBlank() -> t.selectedTitle!!
                else -> "[не знайдено]"
            }
            "${t.originalArtist} – ${t.originalTitle} → $replacement"
        }
        if (replacements.isBlank()) return toast("Ручних замін поки немає")
        val clipboard = getSystemService(CLIPBOARD_SERVICE) as ClipboardManager
        clipboard.setPrimaryClip(ClipData.newPlainText("YTM Importer replacements", replacements))
        toast("Список замін скопійовано")
    }

    private fun updateSummary() {
        val p = playlist ?: return
        val matched = p.tracks.count { it.status in setOf(TrackStatus.MATCHED, TrackStatus.ADDED) }
        val review = p.tracks.count { it.status == TrackStatus.REVIEW }
        val missing = p.tracks.count { it.status in setOf(TrackStatus.MISSING, TrackStatus.FAILED) }
        summaryText.text = "${p.name} • ${p.tracks.size} треків • ✓ $matched  ! $review  × $missing"
    }

    private fun refreshRow(index: Int) {
        runOnUiThread {
            if (index in visibleTracks.indices) adapter.notifyDataSetChanged()
        }
    }
