...
APPLY = ROOT / "scripts/apply-v1447-r3-runtime-lifecycle-r5.py"
...
def fixture():
    root = Path(tempfile.mkdtemp(prefix="ytm-r5-runtime-"))
    grouped = {}
    for op in OPS:
...
def main():
    ...
    for op in OPS:
        ...
        if text.count(op["new"]) != 1:
            raise SystemExit(
                f"FAIL: replacement mismatch: {op['name']}"
            )

    for rel, content in NEW_FILES.items():
        if (root / rel).read_text(encoding="utf-8") != content:
            raise SystemExit(
                f"FAIL: generated file mismatch: {rel}"
            )

    print("PASS: exact replacements/new files")
    ...
    print("PASS: R5 runtime/lifecycle selftest complete")
