...
for f in "$GRADLE" "$SELECTOR" "$DATA" "$R1"; do
  test -f "$f" || fail "missing R1 file: $f"
done

grep -Fq 'versionCode = 73' "$GRADLE" || fail "R1 versionCode 73 missing"
grep -Fq 'versionName = "1.4.38-R1"' "$GRADLE" || fail "R1 versionName missing"
