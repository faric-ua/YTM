...
grep -Fq 'versionName: **1.4.38-R1**' START_HERE_ASSISTANT.md \
  || fail "START_HERE current version missing"
grep -Fq 'versionCode: **73**' START_HERE_ASSISTANT.md \
...
grep -Fq 'Version: 1.4.38-R1' PROJECT_STATUS.txt \
  || fail "PROJECT_STATUS version drift"
grep -Fq 'Version code: 73' PROJECT_STATUS.txt \
...
grep -Fq 'v1.4.38-R1 — Checkbox Alignment + Restore Rotation' BACKLOG.md \
  || fail "BACKLOG current v1.4.38-R1 release missing"
grep -Fq 'v1.4.38-R1 — checkbox alignment + Restore rotation' START_HERE_ASSISTANT.md \
  || fail "START_HERE v1.4.38-R1 handoff missing"
