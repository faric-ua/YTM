...
grep -Fq 'versionName: **1.4.47**' START_HERE_ASSISTANT.md \
  || fail "START_HERE current version missing"
grep -Fq 'versionCode: **87**' START_HERE_ASSISTANT.md \
...
grep -Fq 'feat/v1.4.47-playlist-hub' CURRENT_HANDOFF.md \
  || fail "CURRENT_HANDOFF active v1.4.47 branch missing"
grep -Fq 'Exact next execution step' CURRENT_HANDOFF.md \
  || fail "CURRENT_HANDOFF next-action section missing"
grep -Fq 'UX-019 Phase 2 — Playlist Hub + Clean Home' CURRENT_HANDOFF.md \
  || fail "CURRENT_HANDOFF UX-019 Phase 2 focus missing"
...
grep -Fq 'Version: 1.4.47' PROJECT_STATUS.txt \
  || fail "PROJECT_STATUS version drift"
grep -Fq 'Version code: 87' PROJECT_STATUS.txt \
...
grep -Fq 'v1.4.47 — UX-019 Phase 2 Playlist Hub + Clean Home' BACKLOG.md \
  || fail "BACKLOG current v1.4.47 release missing"
grep -Fq 'UX-019 Home Layout Prototype Alignment — Phase 1' START_HERE_ASSISTANT.md \
  || fail "START_HERE UX-019 Phase 2 focus missing"
