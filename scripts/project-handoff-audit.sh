...
grep -Fq 'versionName: **1.4.32**' START_HERE_ASSISTANT.md \
  || fail "START_HERE current version missing"
grep -Fq 'versionCode: **66**' START_HERE_ASSISTANT.md \
...
grep -Fq 'Version: 1.4.32' PROJECT_STATUS.txt \
  || fail "PROJECT_STATUS version drift"
grep -Fq 'Version code: 66' PROJECT_STATUS.txt \
...
grep -Fq 'BUG-002/Q-002 FIX IMPLEMENTED — PHONE RETEST NEEDED v1.4.32' PROJECT_STATUS.txt \
