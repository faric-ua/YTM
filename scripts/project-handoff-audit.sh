...
grep -Fq 'versionName: **1.4.36**' START_HERE_ASSISTANT.md \
  || fail "START_HERE current version missing"
grep -Fq 'versionCode: **70**' START_HERE_ASSISTANT.md \
...
grep -Fq 'Version: 1.4.36' PROJECT_STATUS.txt \
  || fail "PROJECT_STATUS version drift"
grep -Fq 'Version code: 70' PROJECT_STATUS.txt \
...
grep -Fq 'v1.4.36 — Saved File Destinations' BACKLOG.md \
  || fail "BACKLOG current v1.4.36 release missing"
grep -Fq 'UX-008 Phase 2A' START_HERE_ASSISTANT.md \
  || fail "START_HERE UX-008 Phase 2A handoff missing"
