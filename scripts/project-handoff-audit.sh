...
grep -Fq 'versionName: **1.4.45**' START_HERE_ASSISTANT.md \
  || fail "START_HERE current version missing"
grep -Fq 'versionCode: **85**' START_HERE_ASSISTANT.md \
...
grep -Fq 'feat/v1.4.45-title-emphasis' CURRENT_HANDOFF.md \
  || fail "CURRENT_HANDOFF active v1.4.45 branch missing"
grep -Fq 'Exact next execution step' CURRENT_HANDOFF.md \
  || fail "CURRENT_HANDOFF next-action section missing"
grep -Fq 'UX-022 Unified Window Title Emphasis' CURRENT_HANDOFF.md \
  || fail "CURRENT_HANDOFF UX-022 focus missing"
...
grep -Fq 'Version: 1.4.45' PROJECT_STATUS.txt \
  || fail "PROJECT_STATUS version drift"
grep -Fq 'Version code: 85' PROJECT_STATUS.txt \
