...
grep -Fq 'versionName: **1.4.44-R1**' START_HERE_ASSISTANT.md \
  || fail "START_HERE current version missing"
grep -Fq 'versionCode: **84**' START_HERE_ASSISTANT.md \
...
grep -Fq 'fix/v1.4.44-r1-action-copy-close-style' CURRENT_HANDOFF.md \
...
grep -Fq 'Version: 1.4.44-R1' PROJECT_STATUS.txt \
  || fail "PROJECT_STATUS version drift"
grep -Fq 'Version code: 84' PROJECT_STATUS.txt \
