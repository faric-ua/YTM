...
grep -Fq 'versionName: **1.4.42**' START_HERE_ASSISTANT.md \
  || fail "START_HERE current version missing"
grep -Fq 'versionCode: **80**' START_HERE_ASSISTANT.md \
...
grep -Fq 'Version: 1.4.42' PROJECT_STATUS.txt \
  || fail "PROJECT_STATUS version drift"
grep -Fq 'Version code: 80' PROJECT_STATUS.txt \
...
grep -Fq 'v1.4.42 — Recent File Selector' BACKLOG.md \
  || fail "BACKLOG current v1.4.42 release missing"
grep -Fq 'UX-020 + UX-008 Phase 2B' START_HERE_ASSISTANT.md \
